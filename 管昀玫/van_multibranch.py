...
class MultiScaleBranch(nn.Module):
    ...
    def forward(self, x):
        ...
        out_7 = self.LKA7(x)
        out_9 = self.LKA9(x)
        # cat = torch.cat([out_3, out_5, out_7], dim=1)
        return torch.cat([out_5, out_7, out_9], dim=1)
...
class Attention(nn.Module):
    def __init__(self, d_model):
        ...
        self.spatial_gating_unit = MultiScaleBranch(d_model)
        self.proj_2 = nn.Conv2d(d_model*3, d_model, 1)
...
@register_model
def van_b0_multibranch(pretrained=False, **kwargs):
    model = VAN(
