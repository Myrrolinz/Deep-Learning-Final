...
class ResNet(nn.Module):
    ...
    def __init__(self, block, blocks_num, num_classes=1000, include_top=True,att_type=None):
        ...
        # Residual blocks
        self.layer1 = self._make_layer(block, 64, blocks_num[0],att_type=att_type)
        self.layer2 = self._make_layer(block, 128, blocks_num[1], stride=2,att_type=att_type)
        self.layer3 = self._make_layer(block, 256, blocks_num[2], stride=2,att_type=att_type)
        self.layer4 = self._make_layer(block, 512, blocks_num[3], stride=2,att_type=att_type)
        if self.include_top:
    ...
    def _make_layer(self, block, channel, block_num, stride=1,att_type=None):
        downsample = None
...
def resnet18(num_classes=1000, include_top=True,att_type=None):
    return ResNet(BasicBlock, [2, 2, 2, 2], num_classes=num_classes, include_top=include_top, att_type=att_type)

def resnet34(num_classes=1000, include_top=True,att_type=None):
    return ResNet(BasicBlock, [3, 4, 6, 3], num_classes=num_classes, include_top=include_top,att_type=att_type)

def resnet101(num_classes=1000, include_top=True,att_type=None):
    return ResNet(Bottleneck, [3, 4, 23, 3], num_classes=num_classes, include_top=include_top,att_type=att_type)
