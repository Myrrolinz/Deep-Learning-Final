...
class TripletLKAAttention(nn.Module):
    def __init__(
        self,
        gate_channels,
        reduction_ratio=16,
        ...
        super(TripletLKAAttention, self).__init__()
        ...
        self.c_type=c_type
        if c_type=="LKA":
            self.SpatialGate=LKA(gate_channels)
        elif c_type=="CA":
            self.SpatialGate=CA.CoordAtt(inp=gate_channels,oup=gate_channels,reduction=reduction_ratio)
        elif c_type=="triplet":
